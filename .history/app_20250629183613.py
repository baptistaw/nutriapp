...
def patient_shopping_list_page():
    """
    Generates and displays the shopping list for the patient's latest plan.
    """
    patient = g.patient
    app.logger.info(f"Generando lista de compras para Paciente ID {patient.id}")
    ...
    if not latest_evaluation or not latest_evaluation.edited_plan_text:
        flash("No se encontró un plan para generar la lista de compras.", "info")
        return render_template('patient_shopping_list.html', patient=patient, shopping_list_items={}, evaluation=None)
    ...
    if not recetario_block_text:
        app.logger.warning(f"No se encontró recetario en el plan para Paciente ID {patient.id}, Evaluación ID {latest_evaluation.id}")
        return render_template('patient_shopping_list.html', patient=patient, shopping_list_items={}, evaluation=latest_evaluation)
    ...
    app.logger.info(f"Lista de compras generada para Paciente ID {patient.id} con {len(shopping_list_items)} items.")
    return render_template('patient_shopping_list.html', patient=patient, shopping_list_items=shopping_list_items, evaluation=latest_evaluation)
