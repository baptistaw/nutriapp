...
# login_manager = LoginManager() # Configurar más adelante
# login_manager.init_app(app)
# login_manager.login_view = 'login' # Ruta de login (a crear)
...
try:
    if app.config['GEMINI_API_KEY']:
        ...
        print("INFO: API Key de Gemini configurada.")
    else:
        print("ADVERTENCIA: GEMINI_API_KEY no configurada en el archivo .env.")
...
# Modelo User (Básico para futura implementación de login)
# class User(UserMixin, db.Model):
#     id = db.Column(db.Integer, primary_key=True)
#     username = db.Column(db.String(64), index=True, unique=True)
#     email = db.Column(db.String(120), index=True, unique=True)
#     password_hash = db.Column(db.String(128))
#     # Relación con las evaluaciones que creó
#     evaluations = db.relationship('Evaluation', backref='author', lazy='dynamic')
#
#     def set_password(self, password):
#         # Implementar hashing de contraseña (ej. werkzeug.security)
#         pass
...
class Evaluation(db.Model):
    ...
    # user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True, index=True) # Para login futuro
...
class Ingredient(db.Model):
    ...
    # Relación con los datos nutricionales de este ingrediente
    nutrients = db.relationship('IngredientNutrient', backref='ingredient', lazy=True)
...
class UserPreparation(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    # user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True) # Descomentar si tienes tabla User
    ...
    # user = db.relationship('User', backref=db.backref('preparations', lazy=True)) # Descomentar si tienes User
...
def _clean_item_name_further(name_str: str) -> str:
    ...
    # Remove trailing parenthetical explanations like (merluza, bacalao, etc.) or (opcional)
    cleaned_name = re.sub(r"\s*\([^)]*\)$", "", name_str).strip()
    # Remove common trailing adjectives/phrases. Order matters sometimes.
    # This list needs to be curated carefully to avoid removing parts of actual ingredient names.
    trailing_phrases = [
        "cocido", "cocida", "crudo", "cruda", "picado", "picada",
        "en cubos", "fresco", "fresca", "entero", "entera",
        "mediano", "mediana", "pequeño", "pequeña", "grande",
        "maduro", "madura", "rallado", "rallada", r"etc\.", r"aprox\."   ]
    for phrase in trailing_phrases:
        # Match phrase at the end of the string, possibly preceded by a comma and/or space
        cleaned_name = re.sub(rf"[,]?\s*{re.escape(phrase)}(\s|$)", " ", cleaned_name, flags=re.IGNORECASE).strip()
    
    # Remove leading "de " if the remaining string is not just a single word after "de"
    # (to protect "de cacao", "de almendras" if they are the full item)
...
def _parse_ingredient_line(line_text_with_star: str) -> dict:
    ...
    text_to_parse = line_text_with_star.lstrip('*').strip() # Quitar solo asterisco inicial, luego strip normal
    ...
    # Attempt 1: Find standardized quantity/unit in parentheses, typically at the end or before another comment.
    # e.g., "item name (100 g)", "item name (aprox. 100g)", "item name (100g) (optional comment)"
    std_pattern_match = re.search(
        r"\((?:aprox\.\s*)?([\d.,/\s]+)\s*([a-zA-ZáéíóúÁÉÍÓÚñÑ]+)\s*\)", 
        text_to_parse, 
        re.IGNORECASE
    )

    if std_pattern_match:
        qty_str = std_pattern_match.group(1).strip()
        unit_str = std_pattern_match.group(2).strip()
        try:
            if '/' in qty_str:
            ...
            if unit_lower in ['gramos', 'gr', 'grs', 'grm']: unit = 'g'
            elif unit_lower in ['mililitros', 'cc', 'mls', 'mlt']: unit = 'ml'
            elif unit_lower in ['kilos', 'kg', 'kgs']: quantity *= 1000; unit = 'g'
            elif unit_lower in ['litros', 'lts', 'l', 'litro']: quantity *= 1000; unit = 'ml'
            else: unit = unit_str # Keep original if not in normalization list

            # Item name is everything before the matched standardized quantity/unit
            item_name_base = text_to_parse[:std_pattern_match.start()].strip()
            # Further clean this base name
            item_name = _clean_item_name_further(item_name_base)
            app.logger.info(f"_parse_ingredient_line (Std Pattern): Item='{item_name}', Qty={quantity}, Unit='{unit}' from '{original_line}'")
            return {'item': item_name, 'quantity': quantity, 'unit': unit}
        except ValueError:
            app.logger.warning(f"_parse_ingredient_line - ValueError parsing std quantity '{qty_str}' from '{original_line}'. Proceeding to next patterns.")
            # Reset for next attempts if parsing failed, but keep original text_to_parse
            quantity = 0.0; unit = "N/A"; item_name = text_to_parse

    # Attempt 2: Find leading household quantity/unit if no std parenthesized one was successfully parsed.
    # e.g., "1/2 aguacate maduro", "2 cucharadas de aceite", "150g de camarones"
    household_pattern_match = re.match(
        r"([\d.,/\s]+)\s*([a-zA-ZáéíóúÁÉÍÓÚñÑ]+(?:\s+de)?)\s+(.+)", 
        text_to_parse, 
        re.IGNORECASE
    )
    if household_pattern_match:
        qty_str = household_pattern_match.group(1).strip()
        unit_str_with_de = household_pattern_match.group(2).strip() # e.g., "taza de", "g"
        item_name_base = household_pattern_match.group(3).strip()
        try:
            if '/' in qty_str:
                num, den = map(float, qty_str.split('/'))
                parsed_quantity = num / den if den != 0 else 0.0
            ...
            unit_str = unit_str_with_de.lower().replace(" de", "").strip() # Remove " de" and normalize
            if unit_str in ['gramos', 'gr', 'grs', 'grm']: unit = 'g'
            elif unit_str in ['mililitros', 'cc', 'mls', 'mlt']: unit = 'ml'
            elif unit_str in ['kilos', 'kg', 'kgs']: quantity *= 1000; unit = 'g'
            elif unit_str in ['litros', 'lts', 'l', 'litro']: quantity *= 1000; unit = 'ml'
            elif unit_str in ['cucharadita', 'cdta', 'cucharaditas']: unit = 'cucharadita'
            elif unit_str in ['cucharada', 'cda', 'cucharadas']: unit = 'cucharada'
            elif unit_str in ['taza', 'tz', 'tazas']: unit = 'taza'
            elif unit_str in ['unidad', 'unidades', 'unid', 'u']: unit = 'unidad'
            elif unit_str in ['pieza', 'piezas', 'pz']: unit = 'pieza'
            elif unit_str in ['filete', 'filetes']: unit = 'filete'
            else: unit = unit_str_with_de # Keep original (with "de" if present) if not in list

            item_name = _clean_item_name_further(item_name_base)
            app.logger.info(f"_parse_ingredient_line (Household Pattern): Item='{item_name}', Qty={quantity}, Unit='{unit}' from '{original_line}'")
            return {'item': item_name, 'quantity': quantity, 'unit': unit}
        except ValueError:
            app.logger.warning(f"_parse_ingredient_line - ValueError parsing household quantity '{qty_str}' from '{original_line}'. Proceeding to simple.")
            quantity = 0.0; unit = "N/A"; item_name = text_to_parse # Reset

    # Attempt 3: Handle "al gusto" / "una pizca" or just a plain item name
    simple_match = re.match(r"(.+?)(?:\s+al gusto|,?\s*una pizca)?$", text_to_parse, re.IGNORECASE)
    if simple_match:
        item_name = _clean_item_name_further(simple_match.group(1).strip())
        # quantity remains 0.0, unit remains "N/A"
        app.logger.info(f"_parse_ingredient_line (Simple/Al Gusto): Item='{item_name}' from '{original_line}'")
    else: # Fallback if even simple pattern fails (should be rare)
        item_name = _clean_item_name_further(text_to_parse) # Clean the whole text as item
        # Clean common "al gusto", "una pizca"
        app.logger.info(f"_parse_ingredient_line (Fallback): Item='{item_name}' from '{original_line}'")

    return {'item': item_name, 'quantity': quantity, 'unit': unit}

def parse_recipe_from_text(recipe_name, full_plan_text):
    """
    Parses a specific recipe's ingredients and instructions from the full AI-generated text.
    Assumes the text includes a '== RECETARIO DETALLADO ==' section and recipes
    are formatted as 'Receta N°X: Recipe Name'.
    
    Attempts to parse ingredient lines like '* 1 taza (150g) Arroz integral'.
    
    Returns a dict {'ingredients': [], 'instructions': '', 'description': ''}
    or None if the recipe is not found or parsing fails.
    """
    if not recipe_name or not full_plan_text:
        return None
    ...
    # Find the specific recipe block
    escaped_recipe_name = re.escape(recipe_name)
    recipe_pattern = re.compile(
        rf"Receta N°\d+:\s*{escaped_recipe_name}\s*\n(.*?)(?=\nReceta N°\d+:|\Z)", 
    ...
    if not recipe_match:
        app.logger.warning(f"Recipe '{recipe_name}' not found in RECETARIO DETALLADO section or format unexpected.")
        return None
    ...
    ingredients_list = []
    instructions_text = ""
    description_text = "" # Puedes intentar parsear una descripción si la IA la incluye
    ...
    if ingredients_match:
        ...
        for line in ingredients_block.split('\n'):
            line = line.strip()
            if not line.startswith('*'): # Ignorar líneas que no son de ingredientes
                continue
            
            parsed_ingredient = _parse_ingredient_line(line)
    ...
    return {
        'name': recipe_name, # Añadir el nombre de la receta parseado aquí
...
def get_ingredient_nutritional_info(ingredient_item_name, quantity, unit):
    ...
    normalized_name = ingredient_item_name.lower().strip()
    ingredient = Ingredient.query.filter(Ingredient.name.ilike(normalized_name)).first()

    if not ingredient:
        app.logger.warning(f"Datos nutricionales no encontrados en BD para ingrediente: '{ingredient_item_name}'")
        return {"calories": 0, "protein_g": 0, "carb_g": 0, "fat_g": 0, "micros": {}}

    # Buscar datos nutricionales de referencia (ej. por 100g)
    # Podrías tener lógica más compleja para elegir la mejor referencia si hay varias
    nutri_ref = IngredientNutrient.query.filter_by(ingredient_id=ingredient.id, reference_unit='g', reference_quantity=100.0).first()
    # Si no hay por 100g, buscar otra referencia (ej. por unidad)
    if not nutri_ref:
         nutri_ref = IngredientNutrient.query.filter_by(ingredient_id=ingredient.id).first()

    if not nutri_ref:
        app.logger.warning(f"No se encontraron datos nutricionales de referencia para ingrediente: '{ingredient_item_name}' (ID: {ingredient.id})")
        return {"calories": 0, "protein_g": 0, "carb_g": 0, "fat_g": 0, "micros": {}}

    # Convertir la cantidad dada a la unidad de referencia de nutri_ref
    # ESTA LÓGICA DE CONVERSIÓN ES CRÍTICA Y DEBE SER ROBUSTA
    ...
    if quantity_in_ref_unit is None or nutri_ref.reference_quantity <= 0:
         app.logger.warning(f"No se pudo convertir cantidad/unidad '{quantity} {unit}' a unidad de referencia '{nutri_ref.reference_unit}' para '{ingredient_item_name}'.")
         return {"calories": 0, "protein_g": 0, "carb_g": 0, "fat_g": 0, "micros": {}}

    # Calcular nutrición escalada
    factor = quantity_in_ref_unit / nutri_ref.reference_quantity
    scaled_micros = {k: (v * factor if isinstance(v, (int, float)) else v) for k, v in nutri_ref.get_micronutrients().items()} # Escalar micros numéricos

    return {
        "calories": round(nutri_ref.calories * factor, 2),
        "protein_g": round(nutri_ref.protein_g * factor, 2),
        "carb_g": round(nutri_ref.carb_g * factor, 2),
        "fat_g": round(nutri_ref.fat_g * factor, 2),
...
def convert_quantity_to_reference_unit(ingredient_id, quantity, unit, reference_unit):
    ...
    # Normalizar unidades de entrada y referencia a minúsculas y sin espacios extra
    unit_norm = unit.lower().strip()
    ref_unit_norm = reference_unit.lower().strip()

    # 1. Normalización básica de unidades comunes de peso/volumen
    common_unit_map = {
        **{k: 'g' for k in ['gramos', 'gr', 'grs', 'grm']},
        **{k: 'ml' for k in ['mililitros', 'cc', 'mls', 'mlt']},
        **{k: 'kg' for k in ['kilos', 'kgs']}, # Se convertirá a 'g' luego
        **{k: 'l' for k in ['litros', 'lts', 'litro']}  # Se convertirá a 'ml' luego
    }
    unit_norm = common_unit_map.get(unit_norm, unit_norm)
    ref_unit_norm = common_unit_map.get(ref_unit_norm, ref_unit_norm)
    ...
    # 2. Si las unidades ya coinciden después de la normalización básica
    if unit_norm == ref_unit_norm:
        return quantity # Ya está en la unidad de referencia

    # 3. Intentar conversión usando la tabla UnitEquivalence (principalmente para convertir unidades caseras a gramos)
    # Las unidades en `household_unit` de la BD deben estar normalizadas (ej. minúsculas, singular)
    # y coincidir con lo que produce `_parse_ingredient_line`.
    ...
    if equivalence_entry and equivalence_entry.grams_per_unit is not None:
        ...
        if ref_unit_norm == 'g':
            ...
            # Conversión de gramos (obtenidos de unidad casera) a ml (unidad de referencia) requiere densidad.
            app.logger.warning(f"convert_quantity: IngID {ingredient_id}. Se convirtieron {unit} a {grams_from_household}g, pero la unidad de referencia es 'ml'. Se necesita densidad para conversión g -> ml.")
            return None
        # Si ref_unit_norm es 'unidad', etc., y obtuvimos gramos, es un desajuste.

    # 4. Manejar casos donde la unidad de entrada ya es 'g' o 'ml' pero no coincide con ref_unit_norm
    if unit_norm == 'g': # y ref_unit_norm no es 'g' (ya cubierto en el paso 2)
        if ref_unit_norm == 'ml':
            app.logger.warning(f"convert_quantity: IngID {ingredient_id}. Entrada es {quantity}g, referencia es 'ml'. Se necesita densidad.")
            return None
        # Si ref_unit_norm es 'unidad', etc., es un desajuste.

    if unit_norm == 'ml': # y ref_unit_norm no es 'ml'
        if ref_unit_norm == 'g':
            app.logger.warning(f"convert_quantity: IngID {ingredient_id}. Entrada es {quantity}ml, referencia es 'g'. Se necesita densidad.")
            return None
        # Si ref_unit_norm es 'unidad', etc., es un desajuste.

    # 5. Manejar 'unidad' como unidad de referencia
    # Esto asume que si la referencia es por 'unidad', la entrada también debería ser una 'unidad' o similar.
    if ref_unit_norm == 'unidad':
        # _parse_ingredient_line normaliza 'unidad', 'unidades', 'unid', 'u', 'pieza', 'filete' a formas singulares.
        # Asegúrate que estas formas coincidan con lo que esperas aquí.
        if unit_norm in ['unidad', 'pieza', 'filete']: # Añade otras formas normalizadas si es necesario
            return quantity # Asume que la cantidad ya está en la unidad correcta.
        else:
            app.logger.warning(f"convert_quantity: IngID {ingredient_id}. Unidad de referencia es 'unidad', pero la unidad de entrada es '{unit_norm}'. No se puede convertir directamente.")
            return None
...
def calculate_total_nutritional_info(ingredients_list):
    ...
    if not isinstance(ingredients_list, list):
        app.logger.error(f"calculate_total_nutritional_info recibió un tipo inválido para ingredients_list: {type(ingredients_list)}")
        return {"calories": 0.0, "protein_g": 0.0, "carb_g": 0.0, "fat_g": 0.0, "micros": {}}

    for ingredient in ingredients_list:
        if isinstance(ingredient, dict):
            item_name = ingredient.get('item')
            quantity_str = ingredient.get('quantity') # Quantity might come as string
            unit = ingredient.get('unit')
            
            try: quantity = float(quantity_str) if quantity_str is not None else 0.0
            except (ValueError, TypeError): quantity = 0.0

            nutri_info = get_ingredient_nutritional_info(item_name, quantity, unit)
...
def generar_estructura_plan_prompt(plan_input_data):
    ...
    **INSTRUCCIONES PARA LA GENERACIÓN DE LA ESTRUCTURA DEL PLAN:**
    ...
    2.  **Contenido de Comidas:**
        ...
        *   **La distribución de macronutrientes diaria DEBE COINCIDIR ESTRICTAMENTE con los Macros Objetivo (Sección D).**
        *   **TODAS las sugerencias de comidas, desde el desayuno hasta la cena, deben ser COHERENTES con el Tipo de Dieta Base (Sección D), APETECIBLES, CULINARIAMENTE LÓGICAS, VARIADAS y presentadas de forma atractiva, no como meras listas de ingredientes aislados.**
    ...
    5.  **Restricciones CLAVE:**
        ...
        *   **CUMPLIMIENTO ESTRICTO DE LÍMITES DE MICRONUTRIENTES (Sección E).**
    ...
    **Genera el BORRADOR de la ESTRUCTURA del plan nutricional semanal ahora, siguiendo TODAS estas instrucciones al pie de la letra. Enfócate en la VARIEDAD de los nombres de los platos, el cumplimiento de todas las restricciones y la eliminación total de cualquier texto que no sea el plan de comidas.**
    """
    return prompt
...
def parse_all_recipes_from_text_block(recetario_text):
    ...
    for i, block_text_raw in enumerate(recipe_blocks_raw):
        ...
        ingredients_section_match = re.search(r"Ingredientes\s*(?:\([^)]*\))?:\s*\n(.*?)(?=\n(?:Preparación:|Condimentos Sugeridos:|Sugerencia de Presentación:)|$)", content_after_title, re.DOTALL | re.IGNORECASE)
        ...
        prep_match = re.search(r"Preparación:\s*\n(.*?)(?=\n(?:Condimentos Sugeridos:|Sugerencia de Presentación:)|$)", content_after_title, re.DOTALL | re.IGNORECASE)
        if prep_match:
        ...
        condiments_match = re.search(r"Condimentos Sugeridos:\s*(.*?)(?=\n(?:Sugerencia de Presentación:)|$)", content_after_title, re.DOTALL | re.IGNORECASE)
        if condiments_match:
        ...
        if recipe_name and (ingredients_list_for_pdf or instructions_text): 
            ...
            app.logger.warning(f"Receta '{recipe_name}' omitida por falta de nombre, ingredientes o instrucciones.")
...
def crear_pdf_v2(evaluation_instance):
    ...
    y_margin = 45
    line_height = 13
    max_width = width - (2 * x_margin) # Ancho máximo para texto
    current_y = height - y_margin

    parsed_recipes = [] # Inicializar parsed_recipes aquí
    ...
    try:
        ...
        current_y -= line_height * 2.5 # Más espacio después de la cabecera

        def draw_section_title(title):
             nonlocal current_y
             if current_y < y_margin + line_height * 4: # Espacio mínimo para título + línea + 1 línea de contenido
                 p.showPage()
                 current_y = height - y_margin
             p.setFont("Helvetica-Bold", 12) # Título de sección más grande
             p.drawString(x_margin, current_y, title)
             current_y -= line_height * 0.6 # Espacio para la línea
             p.line(x_margin, current_y, width - x_margin, current_y) # Línea horizontal
             current_y -= line_height * 1.5 # Más espacio después del título y línea
             p.setFont("Helvetica", 9.5) # Fuente para el contenido de la sección

        def draw_line(text, indent=10):
            nonlocal current_y
            # Usar el tamaño de fuente actual para simpleSplit
            lines = simpleSplit(text, p._fontname, p._fontsize, max_width - indent)
            for line in lines:
                 if current_y < y_margin + line_height: p.showPage(); p.setFont(p._fontname, p._fontsize); current_y = height - y_margin # Mantener fuente actual
                 p.drawString(x_margin + indent, current_y, line.strip()) # Strip leading/trailing whitespace from line segments
                 current_y -= line_height * 1.1 # Un poco más de espaciado entre líneas
            
        draw_section_title("I. Datos del Paciente")
        draw_line(f"Nombre: {patient.name} {patient.surname} (C.I.: {patient.cedula})")
        draw_line(f"F. Nac: {patient.dob.strftime('%d/%m/%Y') if patient.dob else 'N/A'} | Edad: {patient.calculate_age() or 'N/A'} | Sexo: {patient.sex or 'N/A'}")
        draw_line(f"Contacto: {patient.email or 'N/A'} / {patient.phone_number or 'N/A'}")
        draw_line(f"Educación: {patient.education_level or 'N/A'} | P. Adquisitivo: {patient.purchasing_power or 'N/A'}")
        if evaluation_instance.gestational_age_weeks and evaluation_instance.gestational_age_weeks > 0:
             draw_line(f"Edad Gestacional: {evaluation_instance.gestational_age_weeks} semanas")
        draw_line(f"Alergias: {', '.join(patient.get_allergies()) or 'Ninguna'}")
        draw_line(f"Intolerancias: {', '.join(patient.get_intolerances()) or 'Ninguna'}")
        draw_line(f"Preferencias: {', '.join(patient.get_preferences()) or 'Ninguna'}")
        draw_line(f"Aversiones: {', '.join(patient.get_aversions()) or 'Ninguna'}")
        current_y -= line_height * 0.8 # Más espacio después de la sección

        draw_section_title("II. Evaluación Actual")
        draw_line(f"Peso: {evaluation_instance.weight_at_eval or 'N/A'} kg | Altura: {patient.height_cm or 'N/A'} cm | Complexión: {evaluation_instance.calculated_complexion or 'N/A'}")
        draw_line(f"P. Muñeca: {evaluation_instance.wrist_circumference_cm or 'N/A'} cm | P. Cintura: {evaluation_instance.waist_circumference_cm or 'N/A'} cm | P. Cadera: {evaluation_instance.hip_circumference_cm or 'N/A'} cm")
        draw_line(f"Peso Ideal (Est.): {evaluation_instance.calculated_ideal_weight or 'N/A'} kg | GET (Est.): {evaluation_instance.calculated_calories or 'N/A'} kcal (Factor Act: {evaluation_instance.activity_factor or 'N/A'})")
        ...
        draw_line(imc_text)
        if evaluation_instance.imc_risk: draw_risk_indicator(x_margin + 10 + p.stringWidth(imc_text) + 8, current_y + line_height, evaluation_instance.imc_risk)
        draw_line(whr_text)
        if evaluation_instance.whr_risk: draw_risk_indicator(x_margin + 10 + p.stringWidth(whr_text) + 8, current_y + line_height, evaluation_instance.whr_risk)
        draw_line(whtr_text)
        if evaluation_instance.whtr_risk: draw_risk_indicator(x_margin + 10 + p.stringWidth(whtr_text) + 8, current_y + line_height, evaluation_instance.whtr_risk)
        current_y -= line_height * 0.8 # Más espacio después de la sección

        draw_section_title("III. Condiciones Clínicas y Dieta del Plan")
        draw_line(f"Patologías (Evaluación): {', '.join(evaluation_instance.get_pathologies()) or 'Ninguna'}")
        if evaluation_instance.other_pathologies_text: draw_line(f"  Otras Patologías: {evaluation_instance.other_pathologies_text}")
        if evaluation_instance.postoperative_text: draw_line(f"Postoperatorio: {evaluation_instance.postoperative_text}")
        ...
        draw_line(f"Tipo de Dieta Base: {diet_type_pdf}")
        ...
        draw_line(f"Objetivo Peso: {evaluation_instance.target_weight or 'N/A'} kg | Obj. Cintura: {evaluation_instance.target_waist_cm or 'N/A'} cm")
        draw_line(f"Obj. Macros: {macros_obj_pdf}")
        current_y -= line_height * 0.8 # Más espacio después de la sección

        draw_section_title("IV. Plan Semanal Sugerido")
        full_plan_text = evaluation_instance.edited_plan_text or "Plan no disponible."

        # Split plan text into structure and recipes
        recetario_marker = "== RECETARIO DETALLADO =="
        plan_parts = full_plan_text.split(recetario_marker, 1) # Split only on the first occurrence
        ...
        if plan_structure_text:
            p.setFont("Helvetica", 9.5) # Fuente para el plan
            plan_lines = simpleSplit(plan_structure_text, p._fontname, p._fontsize, max_width)
            for line in plan_lines:
                if current_y < y_margin + line_height: p.showPage(); p.setFont(p._fontname, p._fontsize); current_y = height - y_margin # Mantener fuente
                p.drawString(x_margin, current_y, line.strip()) # Strip leading/trailing whitespace
                current_y -= line_height * 1.15 # Más espaciado para el plan
            current_y -= line_height * 1.5 # Más espacio después del plan
        else:
             draw_line("Plan no disponible.")
             current_y -= line_height * 1.5

        if evaluation_instance.user_observations:
            draw_section_title("V. Observaciones Adicionales")
            p.setFont("Helvetica", 9.5) # Fuente para observaciones
            obs_lines = simpleSplit(evaluation_instance.user_observations, p._fontname, p._fontsize, max_width)
            for line in obs_lines:
                 if current_y < y_margin + line_height: p.showPage(); p.setFont(p._fontname, p._fontsize); current_y = height - y_margin # Mantener fuente
                 p.drawString(x_margin, current_y, line.strip()) # Strip leading/trailing whitespace
                 current_y -= line_height * 1.1

        # Render Recetario Detallado (if exists)
        if recetario_block_text:
            p.showPage() # Start recipes on a new page
            current_y = height - y_margin
            draw_section_title("VI. Recetario Detallado") # New section title
            # Aquí iría la lógica para parsear y dibujar las recetas si la tuvieras
            # Por ahora, solo dibujamos el texto crudo del recetario si existe
            p.setFont("Helvetica", 9) # Fuente más pequeña para recetas
            recipe_lines = simpleSplit(recetario_block_text, p._fontname, p._fontsize, max_width)
            for line in recipe_lines:
                if current_y < y_margin + line_height: p.showPage(); p.setFont(p._fontname, p._fontsize); current_y = height - y_margin
                p.drawString(x_margin, current_y, line.strip())
                current_y -= line_height * 1.1
        
        # --- FIN DEL BLOQUE TRY ---
        p.save()
        buffer.seek(0)
        app.logger.info(f"PDF (v2) generado para Evaluación ID: {evaluation_instance.id}")
        return buffer
        ...
        return io.BytesIO() # Devuelve buffer vacío en caso de error
...
def crear_pdf_paciente(evaluation_instance):
    ...
    try:
        ...
        current_y -= line_height_base * 3 # Más espacio después de la cabecera

        def draw_patient_section_title(title):
            nonlocal current_y
            # Comprobar si hay suficiente espacio para el título y al menos una línea de contenido
            if current_y < y_margin + line_height_base * 4:
                p.showPage()
                current_y = height - y_margin
            p.setFont("Helvetica-Bold", 15)
            p.setFillColor(colors.HexColor("#003366")) # Un azul oscuro
            p.drawString(x_margin, current_y, title)
            current_y -= line_height_base * 1.8 # Más espacio después del título
            p.setFont("Helvetica", 10.5)
            p.setFillColor(colors.black)

        def draw_patient_text_block(text_block, indent=0, font_name="Helvetica", font_size=10.5, line_spacing_factor=1.2):
            nonlocal current_y
            p.setFont(font_name, font_size)
            lines = simpleSplit(text_block, p._fontname, p._fontsize, max_width - indent)
            if not lines: return # Don't draw anything if text_block is empty or only whitespace
            for line in lines:
                if current_y < y_margin + line_height_base: # Usar line_height_base para el salto
                    p.showPage()
                    current_y = height - y_margin
                    p.setFont(font_name, font_size) # Re-aplicar fuente
                p.drawString(x_margin + indent, current_y, line.strip()) # Strip leading/trailing whitespace
                current_y -= line_height_base * line_spacing_factor
            current_y -= line_height_base * 0.8 # Más espacio después del bloque
        ...
        # --- Parsear el bloque de recetas detalladas ---
        # Asegúrate de que la función parse_all_recipes_from_text_block esté definida en este archivo
        parsed_recipes = parse_all_recipes_from_text_block(recetario_block_text)

        # Render Plan Structure
        plan_lines = plan_structure_text.split('\n')
        previous_line_was_title = False # Para controlar espaciado antes de títulos de día/comida

        for line_content in plan_lines:
            # Heurística simple para detectar días de la semana o comidas principales y darles estilo
            line_content_lower = line_content.strip().lower()
            is_day_or_meal_title = any(keyword in line_content_lower for keyword in ['lunes:', 'martes:', 'miércoles:', 'jueves:', 'viernes:', 'sábado:', 'domingo:', 'desayuno:', 'almuerzo:', 'cena:', 'colación', 'merienda:']) and not "(ver receta" in line_content_lower
            is_current_line_title = is_day_or_meal_title and line_content.strip() # Check if the line is non-empty after stripping

            if is_current_line_title and not previous_line_was_title and current_y < (height - y_margin - line_height_base * 2): # No añadir espacio extra si es el primer título en la página
                current_y -= line_height_base * 0.5 # Espacio extra antes de un nuevo título de día/comida

            if is_current_line_title:
                p.setFont("Helvetica-Bold", 11) # Títulos de día/comida más grandes
            else:
                p.setFont("Helvetica", 10) # Contenido del plan (normal)

            text_segments = simpleSplit(line_content, p._fontname, p._fontsize, max_width)
            for segment in text_segments:
                if current_y < y_margin + line_height_base:
                    p.showPage()
                    current_y = height - y_margin - line_height_base
                    if is_current_line_title: p.setFont("Helvetica-Bold", 11) 
                    else: p.setFont("Helvetica", 10) # Re-apply fontl 

            # Esta sección estaba mal indentada y causaba la repetición.
            # La movemos fuera del bucle `for line_content in plan_lines:`
            # if parsed_recipes:
            # ... (código de dibujar recetas) ...
            previous_line_was_title = is_current_line_title # Asegúrate de que esta línea esté al final del bucle de plan_lines

        # --- Renderizar Recetas Detalladas (DESPUÉS de la estructura del plan) ---
        if parsed_recipes:
            # Considerar iniciar las recetas en una nueva página si hay muchas o si el plan es largo
            if current_y < y_margin + line_height_base * 10 or len(plan_lines) > 20: # Heurística simple
                p.showPage()
                current_y = height - y_margin
            ...
            for recipe in parsed_recipes:
                ...
                for recipe in parsed_recipes:
                    ...
                    if recipe.get('condiments'):
                        ...
                        p.drawString(x_margin + 20, current_y, "Condimentos Sugeridos:")
                        current_y -= line_height_base * 1.2
                        p.setFont("Helvetica", 10.5)
                    ...
                    if recipe.get('presentation'):
                        p.setFont("Helvetica-Bold", 10.5)
                        p.drawString(x_margin + 20, current_y, "Sugerencia de Presentación/Servicio:")
                        current_y -= line_height_base * 1.2
                        p.setFont("Helvetica", 10.5)
...
def finalizar_guardar_enviar():
    try:
        ...
        # Verificar si el email ya existe para OTRO paciente (si estamos creando uno nuevo)
        if not plan_data.get('patient_id'): # Solo si es un paciente nuevo
            if email_from_form: # Solo si se proporcionó un email
        ...
        if not re.match(r"^\d{1,15}$", cedula): # Ejemplo: CI/DNI numérico hasta 15 dígitos
        ...
        patient_fields = { 
            'name': plan_data.get('name'), 'surname': plan_data.get('surname'), 'cedula': cedula.strip(), # Asegurar que la cédula no tenga espacios extra
        ...
        if 'sex' in patient_fields_clean:
            sex_value_lower = patient_fields_clean['sex'].lower().strip() # Convertir a minúsculas y quitar espacios
            ...
            patient_fields_clean['sex'] = sex_value_lower # Opcional: guardar en minúsculas en la BD para consistencia
        ...
        if 'height_cm' in patient_fields_clean:
            ...
            if not height_str: # Si es cadena vacía, tratar como None
                patient_fields_clean['height_cm'] = None
            else:
                try:
                    height_val = float(height_str)
                    if not (30 < height_val < 300): # Rango de altura razonable en cm
        ...
        if paciente: 
            ...
            # Si el paciente existe y el email del formulario es diferente y no nulo,
            # verificar que el nuevo email no esté ya en uso por OTRO paciente.
        ...
        v_data = {} # Diccionario para los datos validados de la evaluación
        try:
            ...
            # Validación de Micronutrientes (ejemplos de rangos, ajústalos según necesidad)
            # Asumiendo que los micronutrientes vienen en plan_data['micronutrients'] como un dict
            ...
            # Suma de macros (opcional, pero útil)
            macros_sum = sum(filter(None, [v_data['target_protein_perc'], v_data['target_carb_perc'], v_data['target_fat_perc']]))
            if macros_sum > 0 and not (98 <= macros_sum <= 102): # Permitir un pequeño margen si se definen
                 app.logger.warning(f"Suma de macros ({macros_sum}%) fuera del rango esperado (98-102%).")
                 # Podrías devolver un error aquí si la suma debe ser estrictamente 100
                 # return jsonify({'error': 'La suma de los porcentajes de macronutrientes debe ser aproximadamente 100%.'}), 400
        ...
        nueva_evaluacion = Evaluation(
            ...
            gestational_age_weeks=v_data.get('gestational_age_weeks', 0), # Usar default del modelo si es None
            activity_factor=v_data.get('activity_factor', 1.2), # Usar default del modelo si es None
            
            # Campos calculados (se asume que vienen correctos de /calcular_valores)
        ...
        # Guardar los rangos de referencia calculados en la evaluación
        references_from_frontend = plan_data.get('references')
        app.logger.info(f"Finalizar: Datos de 'references' recibidos del frontend: {references_from_frontend}") # DEBUG LOG
        ...
        # Los micronutrientes ya se establecieron antes del commit si se usa v_data
        ...
        # PDF Completo (para el nutricionista)
        ...
        pdf_buffer_completo = crear_pdf_v2(nueva_evaluacion) # type: ignore
        ...
        if pdf_completo_generado_ok:
             ...
             if drive_file_id:
                  nueva_evaluacion.pdf_storage_path = drive_file_id # Usar el nombre de columna corregido
                  db.session.commit()
        ...
        pdf_buffer_paciente = crear_pdf_paciente(nueva_evaluacion) # type: ignore
        pdf_paciente_generado_ok = pdf_buffer_paciente.getbuffer().nbytes > 0

        # El envío de email ahora se manejará por separado
        ...
        return jsonify({
            ...
            'patient_email': paciente.email, # Devolver email para el frontend
            'pdf_paciente_generado_ok': pdf_paciente_generado_ok # Indicar si el PDF del paciente se generó
        })

    except ValueError as e: # Esto capturará errores de validación de validate_numeric_field y otros ValueErrors
...
def enviar_plan_email_route(evaluation_id): # Nombre de función cambiado para coincidir con el ejemplo anterior
    ...
    try:
        ...
        if email_enviado:
            ...
            app.logger.error(f"Falló el envío de email a {patient.email} para Evaluación ID {evaluation_id}.")
            return jsonify({'error': 'No se pudo enviar el email con el plan al paciente.'}), 500
    except Exception as e:
...
def api_listar_pacientes():
    ...
    # Búsqueda predictiva para la API también
    search_term = f"%{q}%"
    coincidentes = Patient.query.filter(
        Patient.cedula.ilike(search_term) | 
...
if __name__ == '__main__':
    with app.app_context():
        try:
             app.logger.info("Aplicación iniciada (usar 'flask db upgrade' para aplicar migraciones).")
        except Exception as db_init_err:
             app.logger.critical(f"CRÍTICO: Error al inicializar BD: {db_init_err}")
...
def update_user_preparation(preparation_id):
    ...
    if 'ingredients' in data:
        ingredients_list_to_calculate = data['ingredients']
        preparation.set_ingredients(ingredients_list_to_calculate)
    if 'suitability_tags' in data:
        preparation.set_suitability_tags(data['suitability_tags'])
    
    # Recalcular información nutricional
    total_nutri = calculate_total_nutritional_info(ingredients_list_to_calculate)
    current_num_servings = preparation.num_servings if preparation.num_servings > 0 else 1.0
    preparation.calories_per_serving = round(total_nutri['calories'] / current_num_servings, 2)
    preparation.protein_g_per_serving = round(total_nutri['protein_g'] / current_num_servings, 2)
    preparation.carb_g_per_serving = round(total_nutri['carb_g'] / current_num_servings, 2)
    preparation.fat_g_per_serving = round(total_nutri['fat_g'] / current_num_servings, 2)
...
def mis_preparaciones_view():
    ...
    return render_template('mis_preparaciones.html')

@app.route('/api/relevant_preparations_for_patient', methods=['POST'])
# @login_required # Considerar autenticación si es necesario
def get_relevant_preparations_for_patient():
    ...
    # --- Lógica para derivar tags de los datos del paciente ---
    required_tags = set()
    patient_pathologies = data.get('pathologies', []) # Lista de strings de patologías
    
    # Recolectar todas las restricciones alimentarias del paciente
    patient_aversions_raw = data.get('aversions', [])
    patient_allergies_raw = data.get('allergies', []) 
    patient_intolerances_raw = data.get('intolerances', [])

    all_patient_restrictions_lower = set()
    raw_restriction_lists = {
        "aversions": patient_aversions_raw,
        "allergies": patient_allergies_raw,
        "intolerances": patient_intolerances_raw
    }

    for key, list_from_tagify_field in raw_restriction_lists.items():
        # list_from_tagify_field is expected to be a list like:
        # ['[{"value":"tag1"}]', '[{"value":"tag2"}]'] (if each tag is sent as a separate JSON string)
        # OR ['[{"value":"tag1"},{"value":"tag2"}]'] (if all tags for a field are in one JSON string)
        # OR simple strings like ['plain_tag1', 'plain_tag2']
        # OR a mix, or even malformed items like ['{"value":"malformed"}]']
        if isinstance(list_from_tagify_field, list):
            for item_entry in list_from_tagify_field:
                if isinstance(item_entry, str):
                    ...
                    # Attempt to parse if it looks like a JSON array (Tagify's typical output)
                    if processed_value.startswith('[') and processed_value.endswith(']'):
                        try:
                            tags_in_json = json.loads(processed_value) # Parses '[{"value":"tag1"}]' or '[{"value":"tag1"},{"value":"tag2"}]'
                            for tag_obj in tags_in_json: # Iterate through potentially multiple tags in one JSON string
                                if isinstance(tag_obj, dict):
                                    value = tag_obj.get('value')
                                    if value and isinstance(value, str):
                                        all_patient_restrictions_lower.add(value.lower().strip())
                        except json.JSONDecodeError:
                            app.logger.warning(f"Could not decode JSON string for {key}: '{processed_value}'. Adding raw value.")
                            all_patient_restrictions_lower.add(processed_value.lower()) # Add the problematic string itself
                    else:
                        # It's a plain string, not a JSON array
                        all_patient_restrictions_lower.add(processed_value.lower())
                # else: item_entry is not a string, ignore or log if necessary

    # Mapa de generalizaciones para restricciones (aversiones, alergias, intolerancias).
    # Las claves son términos generales (en minúsculas) y los valores son listas de ítems específicos (en minúsculas).
    restriction_generalizations = {
        ...
        "frutos secos": ["nuez", "almendra", "avellana", "pistacho", "anacardo", "marañón", "pecana", "maní", "cacahuete"], # Maní es legumbre pero a menudo agrupado aquí por alergias
        "gluten": ["trigo", "avena", "cebada", "centeno", "espelta", "kamut", "triticale", "harina de trigo", "pan", "pasta", "galletas", "sémola"], # Avena puede ser problemática si no es certificada sin gluten.
        "lactosa": ["leche", "queso", "yogur", "nata", "crema", "mantequilla", "suero de leche", "caseína", "lactosa"],
        "lacteos": ["leche", "queso", "yogur", "nata", "crema", "mantequilla", "suero de leche", "caseína", "lactosa"], # Alias para lactosa/lácteos
        ...
        # Add more specific items like "pollo" or "lentejas" if you want them to also act as general categories
        # or ensure your ingredient data is very specific.
        # For example, if "pollo" should catch "pechuga de pollo", add:
        # "pollo": ["pollo", "pechuga de pollo", "muslo de pollo", "alitas de pollo"],
    }

    # Mapeo simple de patologías a tags (ejemplos, expandir según necesidad)
    # Estos tags deben coincidir con los que se guardan en UserPreparation.suitability_tags_json
    pathology_to_tag_map = {
        "diabetes tipo i": "diabetico",
        "diabetes tipo ii hgo": "diabetico",
        "diabetes tipo ii insulinorequirente": "diabetico",
        "diabetes pregestacional": "diabetico", 
        "diabetes gestacional": "diabetico_gestacional", 
        "diabetes gestacional insulinorequirente": "diabetico_gestacional",
        "dislipidemia": "bajo_colesterol", 
        "hipertensión arterial": "bajo_sodio", 
        "insuf. renal leve-moderada": "renal_leve_mod", 
        "insuf. renal severa": "renal_severo",
    }
    ...
    if data.get('objective_low_cholesterol'): 
        required_tags.add('bajo_colesterol')
    if data.get('objective_low_potassium'):
        required_tags.add('renal_bajo_potasio')

    app.logger.info(f"API relevant_preparations - Tags requeridos: {required_tags}, Restricciones del paciente (combinadas): {all_patient_restrictions_lower}")
    ...
    for prep in all_preparations:
        ...
        if not required_tags.issubset(prep_tags):
            app.logger.debug(f"Prep ID {prep.id} ({prep.name}) SKIPPED (TAGS): Does not meet all required_tags. Prep tags: {prep_tags}, Required: {required_tags}")
            continue

        if all_patient_restrictions_lower:
            skip_prep_due_to_restriction = False
            prep_ingredient_items_lower = {ing.get('item', '').lower().strip() for ing in prep.get_ingredients() if ing.get('item')}
            app.logger.debug(f"Prep ID {prep.id} ({prep.name}) - Checking restrictions. Prep ingredients: {prep_ingredient_items_lower}. Patient restrictions: {all_patient_restrictions_lower}")

            for restriction_item in all_patient_restrictions_lower:
                # Convert the patient's restriction item to lowercase and strip it once
                # This is the string we will search for as a substring.
                search_term = restriction_item # Already lowercased and stripped when all_patient_restrictions_lower was created

                # 2a: Check if the search_term (e.g., "salmón", "pollo", "lentejas") 
                # is a substring of any ingredient OR in the preparation's name or description.
                prep_name_lower = prep.name.lower()
                # prep_description_lower = (prep.description or "").lower() # Optionally include description
                ...
                if any(search_term in prep_ingredient_item for prep_ingredient_item in prep_ingredient_items_lower) or \
                   search_term in prep_name_lower:
                    # Optionally add: or search_term in prep_description_lower
                    ...
                    app.logger.debug(f"Prep ID {prep.id} ('{prep.name}') SKIPPED due to SUBSTRING restriction: '{search_term}' found in prep {location_found}. Ingredients: {prep_ingredient_items_lower}, Name: {prep_name_lower}")
                    skip_prep_due_to_restriction = True
                    break 
                
                # 2b: Check if the search_term is a general category (e.g., "pescado")
                # and if any of its specific items are substrings of any ingredient
                if search_term in restriction_generalizations:
                    specific_items_to_avoid = restriction_generalizations[search_term] # This is a list of strings
                    app.logger.debug(f"Prep ID {prep.id} ({prep.name}) - General restriction category '{search_term}'. Specific items to check as substrings: {specific_items_to_avoid}.")
                    for specific_avoid_item in specific_items_to_avoid:
                        # Now check if this specific_avoid_item (e.g., "salmón") is a substring of any prep ingredient
                        # OR in the preparation's name or description.
                        if any(specific_avoid_item in prep_ingredient_item for prep_ingredient_item in prep_ingredient_items_lower) or \
                           specific_avoid_item in prep_name_lower:
                            # Optionally add: or specific_avoid_item in prep_description_lower
                            ...
                            app.logger.debug(f"Prep ID {prep.id} ('{prep.name}') SKIPPED due to GENERALIZED restriction: category '{search_term}' matched specific item '{specific_avoid_item}' in prep {gen_location_found}. Ingredients: {prep_ingredient_items_lower}, Name: {prep_name_lower}")
                            skip_prep_due_to_restriction = True
                            break # Break from inner loop (specific_items_to_avoid)
        ...
        app.logger.debug(f"Prep ID {prep.id} ('{prep.name}') ADDED. Tags: {prep_tags}, Ingredients: {[ing.get('item') for ing in prep.get_ingredients()]}")
